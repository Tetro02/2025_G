...
// slv_reg1（LITE1_ADDR）: DDS频率字 freq_word[27:0]（28bit相位累加器）
// slv_reg2（LITE2_ADDR）: DDS相位/波形/幅度（适配14bit DAC）
//   [10:0]  = phase_off（11bit相位偏移，0~2047，对应ADDR_W=11的2048点/波形）
//   [12:11] = wave_sel（2bit波形选择）
//   [20:13] = amplitude（8bit幅度，0~255）
...
// ===================== 全局变量 =====================
uint16_t sweep_freq_index = 1; // 频率扫描索引
...
// ==============================================================================
// DDS控制函数（适配14bit DAC + 11bit phase_off）
// 参数：target_freq_hz - 目标频率（Hz）
//       phase_off     - 11bit相位偏移（0~2047，对应2048点/波形）
//       wave_sel      - 波形选择（0:正弦 1:方波 2:三角波 3:锯齿波）
//       amplitude     - 8bit幅度（0~255）
// 使用示例：
//   SetDDS(100000.0f, 0, DDS_WAVE_SIN, 200); // 100kHz正弦波，幅度200
// ==============================================================================
void SetDDS(float target_freq_hz, uint16_t phase_off, uint8_t wave_sel, uint8_t amplitude)
{
    // freq_word = (target_freq * 2^28) / 50_000_000
    ...
    // slv_reg2: [20:13]=amplitude, [12:11]=wave_sel, [10:0]=phase_off
    uint32_t ctrl = ((uint32_t)amplitude << 13) | ((uint32_t)wave_sel << 11) | (uint32_t)(phase_off & 0x07FF);
...
// 中断服务函数（使用统一控制函数）
// ==============================================================================
int PL_IRQHandler(void *CallbackRef) // PL中断服务函数
{
    (void)CallbackRef;
    SetControlFlags(CTRL_BIT_DDS | 0x00); // 停止ADC采样模式，保留DDS状态
    sweep_freq_index++;
    SetDDS(0.0f + (double)sweep_freq_index * 488.28125f, 0, DDS_WAVE_SIN, 255); // 更新DDS频率
    ...
    // printf("=========================================\r\n\r\n");
    if (sweep_freq_index >= 4096)
    {
        printf("Frequency sweep completed.\r\n");
        return 1; // 扫频完成，停止更新频率
    }

    SetControlFlags(CTRL_BIT_DDS | CTRL_BIT_ADC_SAMPLE); // 重启ADC采样模式
}
...
int main(void)
{
    ...
    // 使能DDS模式（bit2置1）
    printf("Starting frequency sweep...\r\n");
    SetControlFlags(CTRL_BIT_DDS | 0x00);                                       // 仅DDS使能，其他功能关闭
    SetDDS(0.0f + (double)sweep_freq_index * 488.28125f, 0, DDS_WAVE_SIN, 255); // 频率从100kHz线性增加
    SetControlFlags(CTRL_BIT_DDS | CTRL_BIT_ADC_SAMPLE);                        // 开启扫频模式：DDS+ADC同时工作
