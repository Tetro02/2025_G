...
#define SKIP_SWEEP        0       // 1=跳过扫频, 用硬编码系数直接校准
#define TEST_FIR_ONLY      1       // 1=只测FIR系数计算, 用硬编码扫频数据, 不碰硬件
...
void compute_fir(int n_points, int n_taps, int16_t *coeffs)
{
    ...
    // DEBUG: h_tmp前后各4个
    printf(\"h_tmp前4: %.1f %.1f %.1f %.1f\\r\\n\", h_tmp[0], h_tmp[1], h_tmp[2], h_tmp[3]);
    printf(\"h_tmp后4: %.1f %.1f %.1f %.1f\\r\\n\", h_tmp[997], h_tmp[998], h_tmp[999], h_tmp[1000]);
    fflush(stdout);
    ...
    printf("FIR 系数: %d tap, min=%d max=%d\r\n", n_taps,
           (int)coeffs[0], (int)coeffs[n_taps - 1]);
    // C 数组格式输出，方便硬编码
    printf("// === 复制以下到代码 ===\r\n");
    printf("static const int16_t saved_fir[%d] = {\r\n", n_taps);
    for (int i = 0; i < n_taps; i++) {
        printf("%6d", (int)coeffs[i]);
        if (i < n_taps - 1) printf(",");
        if ((i & 7) == 7) printf("\r\n");
    }
    printf("\r\n};\r\n");
    printf("// === 复制结束 ===\r\n");
...
int main(void)
{
    ...
    printf("Loaded sweep data: %d pts\r\n", SWEEP_DATA_PTS);
    memcpy(H_amp_out, sweep_Ha, sizeof(sweep_Ha));
    memcpy(H_phase_out, sweep_Hp, sizeof(sweep_Hp));
    compute_fir(SWEEP_DATA_PTS, FIR_TAPS, fir_coeffs);
